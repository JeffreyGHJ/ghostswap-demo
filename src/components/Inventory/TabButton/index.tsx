...
const btnContainer = 'relative text-[16px] sm:text-[22px] font-bold tracking-tight w-[10rem] sm:w-[16rem]'
const button = 'flex items-center justify-center whitespace-nowrap gap-1 sm:gap-2'
const active = ' absolute h-[3px] w-full bg-blue '

const TabButton = ({ text, index, tabIndex, badgeValue }) => {
  return (
    <Tab className={btnContainer}>
      <div className={button}>
        {text} <Badge color="blue">{badgeValue || 0}</Badge>
      </div>
