...
import { clearItemsToSell } from '../../../state/cart'
...
const CartItemsToSell = ({ itemsToSell, sellTotal }) => {
        ...
        <>
          <div id="control-group-row" className="flex items-center justify-between mt-3 ml-3 mb-[.1875rem]">
            <div id="item-count" className="font-bold text-[1.25rem]">
              {'Sell ' + itemsToSell.length + (itemsToSell.length > 1 ? ' NFTs' : ' NFT')}
            </div>
            <div id="clear-items">
          ...
          <div id="total-price" className={totalPriceContainer}>
            <div className="w-2/5">Sell Total:</div>
            <div id="item-price" className="flex items-center justify-end w-[30%]">
              <img id="bnb-icon" className="flex items-center w-4 h-4 mr-2" src={bnbIcon} />
              <div>{sellTotal.toFixed(6)}</div>
            </div>
          </div>
          <div className="flex flex-col w-full">
            {itemsToSell.map((nftData: any, index: number) => (
              <CartItem nftData={nftData} key={index + ':' + nftData.id} type={'SELL'} />
            ))}
          </div>
        </>
      )}
...
export default CartItemsToSell
