...
import { ChainId, ROUTER_ADDRESS } from '@ghostlabsweb3/sdk'
...
export function getRouterContract(chainId: number, provider: Web3Provider, account?: string): Contract {
  return getContract(getRouterAddress(chainId), IUniswapV2Router02ABI, provider, account)
}

export function getArcherRouterContract(chainId: number, provider: Web3Provider, account?: string): Contract {
  return getContract('', ArcherSwapRouterABI, provider, account)
}
