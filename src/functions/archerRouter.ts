...
export abstract class ArcherRouter {
  ...
  public static swapCallParameters(
    ...
    const etherIn = trade.inputAmount.currency === BNB.onChain(ChainId.BSC)
    const etherOut = trade.outputAmount.currency === BNB.onChain(ChainId.BSC)
    // the router does not support both ether in and out
    invariant(!(etherIn && etherOut), 'ETHER_IN_OUT')
    invariant(!('ttl' in options) || options.ttl > 0, 'TTL')
    invariant('ethTip' in options && options.ethTip?.currency === BNB.onChain(ChainId.BSC))
