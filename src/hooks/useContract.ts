...
import {
  ...
  MISTROLL_ADDRESS,
  MULTICALL2_ADDRESS,
  PANCAKE_PROFILE_ADDRESS,
  ROUTER_ADDRESS,
  SOUL_ADDRESS,
  STOP_LIMIT_ORDER_ADDRESS,
  WNATIVE_ADDRESS,
} from '@ghostlabsweb3/sdk'
...
import PANCAKE_PROFILE_ABI from '../constants/abis/pancakeProfile.json'
...
export const usePancakeProfileContract = (withSignerIfPossible?: boolean): Contract | null => {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && PANCAKE_PROFILE_ADDRESS[chainId], PANCAKE_PROFILE_ABI, withSignerIfPossible)
}
