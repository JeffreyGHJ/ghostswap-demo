...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BSC, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.BSC]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BSC, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.BSC]).toEqual(3)
    })
    it('works with non-set chains', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BSC_TEST, blockNumber: 2 }))
